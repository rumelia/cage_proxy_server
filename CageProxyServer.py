...
def recv_all(input_socket):
    ...
    # put everything other than headers in buffer
    buffer = buffer.split(b'\r\n\r\n', 1)[1]
    ...
    if headers.find(b'Transfer-Encoding: chunked') != -1:
        ...
        while True:
            ...
            if chunk_size == 0:
                ...
                chunk_data = b''
                # read until the specified chunk size + 4 for CRLF chars
                for i in range(chunk_size):
                    # add data to chunk from buffer char by char
                    chunk_data += buffer[0:1]
                    # print(chunk_data)
                    print("Received {} bytes".format(len(chunk_data)))
                    # consume each char from buffer after adding it to chunk_data
                    buffer = buffer[1:]
                ...
                # consume CRLF \r\n characters that come before next chunk from buffer
                buffer = buffer.split(b'\r\n', 1)[1]
